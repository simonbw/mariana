...
export function makeTurbulence1D(p: TurbulenceParameters): (x: number) => number {
    return (x: number) => turbulence2D(p, x, simplex1DY);
}

export function makeTurbulence2D(p: TurbulenceParameters): (x: number, y: number) => number {
